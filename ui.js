    ...
    if (type === 'clients') {
        ...
        list.innerHTML = filtered.map(c => {
            ...
            const badgeHtml = c.isArchived
                ...
                : (c.active > 0 ? `<span>${c.active} Active Jobs</span>` : `<span class="inactive">${inactiveDisplay}</span>`);
            ...
            if (c.group || c.website) {
                ...
                if (c.website) parts.push(`<span style="display: inline-flex; align-items: center; gap: 4px;"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#64748b" stroke-width="2.5"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1 4-10z"></path></svg><a href="${actualUrl}" target="_blank" onclick="event.stopPropagation();" style="color: #64748b; text-decoration: none; transition: 0.2s;" onmouseover="this.style.color='#0284c7'" onmouseout="this.style.color='#64748b'">${displayUrl}</a></span>`);
        ...
        // Apply Status Filter
        if (subFilterVal !== 'all') {
            filtered = filtered.filter(r => (r.status || 'Pending').toLowerCase() === subFilterVal);
        }
        ...
        list.innerHTML = filtered.map(r => {
            ...
            let badgeClass = 'new-status'; // default blue
            if (safeClass === 'approved') badgeClass = 'complete-status'; // green
            if (safeClass === 'cancelled') badgeClass = 'invoice-status'; // red

            const dateStr = new Date(r.request_date).toLocaleDateString('en-NZ');
            ...
            <div class="card-item" style="cursor: default;">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; border-top: 1px dashed #e2e8f0; padding-top: 12px;">
                    <div>
                        ${attachHtml || '<span style="font-size: 11px; color: #94a3b8; font-weight: 500;">No attachments</span>'}
                    </div>
