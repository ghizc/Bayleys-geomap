    ...
    import('./ui.js').then(ui => {
        ...
        if (filterType === 'all') {
            ...
            const matchingReports = state.allReportsData.filter(job => {
                ...
                if (filterType === 'active') return !['complete','invoice','pending'].includes(status);
        ...
        import('./map.js').then(mapMod => {
            ...
            if (filteredPremises.length > 0 && state.mapInstance) {
               // Zoom out slightly to see the filtered cluster
               state.mapInstance.flyTo({ center: [filteredPremises[0].lng, filteredPremises[0].lat], zoom: 11 }); 
            }
        });
...
window.connectWFM = () => {
    // PASTE YOUR CLIENT ID HERE (It is 100% safe to have the ID in the frontend)
    const clientId = 'a1350549-4184-4675-9668-71fcf8f50dcc'; 
    
    // This must match exactly what you typed into the WFM setup screen
    ...
    const state = 'bayleys_geomap_auth'; // Security check
    ...
    // Send the user to WorkflowMax to approve access
...
window.refreshAppAdminData = async () => {
    ...
    try {
        ...
        if (state.currentUser && state.currentUser.role === 'admin') {
            ...
            // This is the new, clean call using the updated import
