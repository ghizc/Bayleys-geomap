...
document.getElementById('requestReportForm')?.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        for (const row of rows) {
            ...
            // Only process if the core fields are filled out
            if (addressLine && reportType && deadline) {
                btn.innerText = `Sending Request ${successCount + 1}...`;
                ...
                // Insert this specific row into the database
                const { error: insertError } = await supabase.from('report_requests').insert([payload]);
                if (insertError) throw insertError;
                
                successCount++;

                // Give the Resend email server a 1-second pause so it doesn't block the next email as spam!
                await new Promise(resolve => setTimeout(resolve, 1000));
            }
        }

        if (successCount === 0) throw new Error("Please provide details for at least one premise.");

        log("Report request sent successfully!");
        alert(`Success! ${successCount} report request(s) have been generated and sent to david.guy@bayleysbc.co.nz.`);
        window.closeRequestModal();
    } catch (err) {
