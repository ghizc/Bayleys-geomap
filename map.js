...
function setupCustomUI(parentContainer, containerId, centerArray, zoom) {
    ...
    if (state.currentUser?.id === ADMIN_UUID) {
        ...
        menuContainer.append(btnWFMConnect, btnWFMJobs, btnWFMClientSync, btnMergeTool, document.createElement('hr'), btnSyncLogos, btnFix, btnAutoPremise, btnCleanPremises, btnSharePoint);
        ...
        // MISSING LINK FOUND: We actually have to attach the adminWrapper to the controlsGroup!
        ...
        const disableBtns = (disable) => {
            [btnFix, btnSyncLogos, btnWFMConnect, btnWFMJobs, btnSharePoint, btnAutoPremise, btnCleanPremises, btnWFMClientSync, btnMergeTool].forEach(b => b.disabled = disable);
        ...
        btnSharePoint.onclick = async () => runTool("Syncing SharePoint...", async () => {
            ...
            while (hasMore) {
                ...
                const { data, error } = await supabase.functions.invoke('sharepoint-sync', { 
                    body: { lastJobNumber: currentCursor } // FIXED: Using lastJobNumber!
                });
                ...
                currentCursor = data.nextJobNumber; // FIXED: Using nextJobNumber!
        ...
        btnMergeTool.onclick = () => {
            ...
            // Dynamically inject the new "Merge & Clean" Modal
            const modalHtml = `
            ...
            document.getElementById('quickCreateMasterBtn').onclick = async () => {
                ...
                try {
                    // Call our Edge Function to bypass RLS entirely
                    ...
                    // Add to local state (so it persists until refresh)
                    state.clientsData.push(newClient);
                    
                    // Add to Master dropdown & automatically select it
                    ...
                    // Flash a brief success indication
            ...
            // --- UPDATE "SELECTED" COUNTER & HIGHLIGHT ---
            ...
            // --- ACTION 2: BULK DELETE WITHOUT MERGING ---
    ...
    // Finally, attach everything to the map container!
